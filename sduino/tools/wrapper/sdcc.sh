...
case "$SRC" in
	*.cpp)
		...
		(
			...
			echo "// This file is autogenerated, do not edit it as all changes will be lost on next build."
			echo "// It is created as a workaround to problem with eclipse, sduino and sdcc compiler not like each other :)"
...
#		cp -a "$SRC" "$CSRC"
		"$SDCC" "$@" "$CSRC" -o "$OBJ"
		ERR=$?
		# don't delete the temp file as a workaroud for build problems in eclipse with arduino/sduino plugin
#		rm -f "$CSRC"
		;;
	*.c)
		# compile a .c file
